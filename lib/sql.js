 ...
 */
SQLConnector.prototype.autoupdate = function (models, options, cb) {
  const self = this;
  ...
  assert(typeof models === 'string' || Array.isArray(models), 'models must be a string or an array');
  ...
  async.eachSeries(models, function (model, done) {
    ...
    self.getTableStatus(model, options, function (err, fields, indexes, FKs) {
      if (!err && fields.length) {
        self.alterTable(model, fields, indexes, options, done);
      } else {
        self.createTable(model, options, done);
      }
    });
  }, cb);
};

SQLConnector.prototype.generateAsIsIndexCompare = function (asIsModelIndexes) {
...
SQLConnector.prototype.generateToBeIndexCompare = function(modelIndexes) {
  let toBeIndexes = {};
  if (modelIndexes) {
...
SQLConnector.prototype.isActualIndexes = function (modelName, callback) {
  throw new Error(g.f('{{isActualIndexes()}} must be implemented by the connector'));
}
 ...
 * @param {Function} [cb] The callback function
 */
SQLConnector.prototype.isActual = function (models, options, cb) {
  const self = this;
  ...
  async.each(models, function (model, done) {
    self.getTableStatus(model, options, function (err, fields) {
      changes = changes.concat(self.getAddModifyColumns(model, fields));
      changes = changes.concat(self.getDropColumns(model, fields));
      self.isActualIndexes(model, (err, isActual) => {
        if (!isActual) {
          debug('index changes for model', model);
          changes.push({});
        }
        done(err);
      });
...
SQLConnector.prototype.applySqlChanges = function (model, pendingChanges, options, cb) {
  const self = this;
  ...
  if (pendingChanges.length) {
    ...
    self.execute(thisQuery, [], options, cb);
  }
};
 ...
 * @param {Function} cb The callback function
 */
SQLConnector.prototype.alterTable = function (model, fields, indexes, options, cb) {
 ...
 * @param {Function} cb The callback function
 */
SQLConnector.prototype.getTableStatus = function (model, options, cb) {
  let fields, indexes;
  const self = this;
  ...
  this.showFields(model, options, function (err, data) {
    if (err) return cb(err);
    fields = data;

    self.showIndexes(model, options, function (err, data) {
      if (err) return cb(err);
      indexes = data;
 ...
 * @param {Function} cb The callback function
 */
SQLConnector.prototype.showFields = function (model, options, cb) {
 ...
 * @param {Function} cb The callback function
 */
SQLConnector.prototype.showIndexes = function (model, options, cb) {
 ...
 * @param {Function} [cb] The callback function
 */
SQLConnector.prototype.dropTable = function (model, options, cb) {
  if ((!cb) && typeof options === 'function') {
    cb = options;
    options = {};
  }
  ...
  this.execute('DROP TABLE IF EXISTS ' + this.tableEscaped(model), [], options, cb);
};
 ...
 * @param {Function} [cb] The callback function
 */
SQLConnector.prototype.createTable = function (model, options, cb) {
  if ((!cb) && typeof options === 'function') {
    cb = options;
    options = {};
  }
 ...
 * @param {Function} [cb] The callback function
 */
SQLConnector.prototype.automigrate = function (models, options, cb) {
  const self = this;
...
SQLConnector.prototype.discoverDatabaseSchemas = function (options, cb) {
  ...
  options = options || {};
  const self = this;
  this.execute(self.buildQuerySchemas(options), [], options, cb);
};
...
SQLConnector.prototype.discoverModelDefinitions = function (options, cb) {
  ...
  const calls = [function (callback) {
    self.execute(self.buildQueryTables(options), [], options, callback);
  }];

  if (options.views) {
    calls.push(function (callback) {
      self.execute(self.buildQueryViews(options), [], options, callback);
    });
  }
...
SQLConnector.prototype.discoverModelProperties = function (table, options, cb) {
  ...
  this.execute(sql, [], options, callback);
};
